<?php
	...
	$displayImages = "";
	     $sqluse=mysql_query("select s_id from users where college=(select  college  from  users where  s_id='$_SESSION[valid_uid]')");
		 while($s=mysql_fetch_array($sqluse))
		 {
             $collegesid=$s['s_id'];		 
     	$sqlSelProd = mysql_query("select * from  tblproduct  where s_id='$collegesid'  order by prod_name") or die(mysql_error());
