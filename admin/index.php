<?php
	...
	if($submit == "Log in"){
		...
		if(mysql_num_rows($sqlSelUser) == 1){
			...
			$sqlSelUserInfo = mysql_query("select user_type, lname, mname from tblusers where user_id = '$userid'") or die(mysql_error());
			$getUserInfo = mysql_fetch_array($sqlSelUserInfo);
			$mname = $getUserInfo["lname"];
			$lname = $getUserInfo["mname"];
			$name = $mname." ".$lname;
			$userType = $getUserInfo["user_type"];
			$_SESSION["name"] = $name;
			
			if($userType == "Administrator")
				header("location: admin_change_userpass.php");
